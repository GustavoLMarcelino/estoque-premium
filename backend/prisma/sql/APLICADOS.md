...
| backend/prisma/sql/2026-08-18-garantia-documento-opcional.sql | `96bd3c8d143f` | 18/08/2026 | Gustavo | DDL: garantias.cliente_documento passa de NOT NULL para NULL (VARCHAR(20), utf8mb4_unicode_ci restatados no MODIFY para não herdar o default da tabela). Sem backfill — 1 garantia existente, com documento preenchido, inalterada. Conferido antes/depois: IS_NULLABLE NO→YES, tipo/charset/collation/default/comment preservados, 0 linhas alteradas. |
| backend/prisma/sql/2026-09-19-index-movimentacoes-data.sql | `81cc4ac0d9cb` | 22/09/2026 | Gustavo | CREATE INDEX idx_mov_data em movimentacoes(data_movimentacao) — já existia em produção (criado manualmente antes da abertura do PR #3), confirmado idêntico ao arquivo. Sem alteração de dado, só metadado de índice. |
