...
/** GET /api/estoque?q=&marca_id=&page=&pageSize= */
...
estoqueRouter.post('/', requireAdmin, validate({ body: criarProdutoBody }), async (req, res, next) => {
  try {
    ...
    console.error('POST /api/estoque ERRO:', e);
    next(e);
  }
});
...
estoqueRouter.put('/:id', requireAdmin, validate({ params: idParams, body: editarProdutoBody }), async (req, res, next) => {
  try {
    ...
    console.error('PUT /api/estoque/:id ERRO:', e);
    next(e);
  }
});
...
estoqueRouter.delete('/:id', requireAdmin, validate({ params: idParams }), async (req, res, next) => {
  try {
    ...
    if (e?.code === 'P2003') {
      // FK constraint (existe movimentação vinculada)
      return res.status(409).json({
        error: true,
        message: 'Não é possível remover: existem movimentações vinculadas a este produto.',
      });
    }
    console.error('DELETE /api/estoque/:id ERRO:', e);
    next(e);
  }
});
