...
// Criação de usuários migrou para POST /api/usuarios (admin-only, com
// permissões granulares) — o antigo /register foi absorvido por lá.
...
authRouter.get('/me', requireAuth, async (req, res, next) => {
  try {
    // requireAuth já carregou o usuário fresco do banco (com permissões parseadas).
    const { id, name, email, role, permissoes } = req.user;
    res.json({ user: { id, name, email, role, permissoes } });
  } catch (e) {
    next(e);
  }
});
